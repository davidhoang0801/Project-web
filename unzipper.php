...
if (isset($_POST['dounzip'])) {
  //Kiểm tra xem một kho lưu trữ đã được chọn để giải nén hay chưa
...
class Unzipper {
  ...
  public function __construct() {
    //Đọc thư mục và chọn các tệp .zip, .rar...
  ...
  public function prepareExtraction($archive, $destination = '') {
    ...
    //Chỉ các kho lưu trữ hiện có tại địa phương mới được phép trích xuất
  ...
  public static function extractZipArchive($archive, $destination) {
    //Kiểm tra xem máy chủ web có hỗ trợ giải nén hay không
    ...
    //Kiểm tra xem bản lưu trữ có thể đọc được không
    if ($zip->open($archive) === TRUE) {
      //Kiểm tra xem điểm đến có thể ghi được không
  ...
  public static function extractGzipFile($archive, $destination) {
    //Kiểm tra xem zlib đã được bật chưa
    ...
    if (file_exists($destination . '/' . $filename)) {
      ...
      //Nếu chúng ta có tệp tar.gz, hãy giải nén tệp tar đó
...
class Zipper {
  ...
  private static function folderToZip($folder, &$zipFile, $exclusiveLength) {
    ...
    while (FALSE !== $f = readdir($handle)) {
      //Kiểm tra đường dẫn cục bộ / chính hoặc tệp nén chính nó và bỏ qua
      if ($f != '.' && $f != '..' && $f != basename(__FILE__)) {
        $filePath = "$folder/$f";
        //Xóa tiền tố khỏi đường dẫn tệp trước khi thêm vào zip
